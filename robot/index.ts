...
export class Robot {
    ...
    static createEmbedMessage(title: string, prompt: string, thumbnail: string, items: string[]) {
    ...
    static createArk23Message(items: string[]) {
      const message = {
        template_id: 23, kv: [
          { key: '#LIST#', obj: [] }
        ]
    ...
    async getAllChannelId() {
      const guildResult = await this.client.meApi.meGuilds()
      const guildIds = guildResult.data.map(item => item.id)
      const channelIds: string[] = []
      for (const guildId of guildIds) {
        const channelResult = await this.client.channelApi.channels(guildId)
    ...
    async getMyFirstChannelId() {
      const guildResult = await this.client.meApi.meGuilds()
      const guildId = guildResult.data[0].id
      const channelResult = await this.client.channelApi.channels(guildId)
      const channelId = channelResult.data[0].id
      return channelId
    }
